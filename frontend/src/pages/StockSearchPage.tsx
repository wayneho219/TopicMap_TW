  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      fetch(`/api/stocks/search?q=${encodeURIComponent(query.trim())}`)
        .then(r => r.json())
        .then((data: SearchResult[]) => { setResults(data); setSearching(false) })
        ...
        {query.trim() ? (
          ...
          <div className="px-4 pt-3">
            {searching ? (
              ...
              <div className="text-center text-[#aaa] text-sm py-10">找不到符合的股票</div>
            ) : (
              <>
                <div className="text-[#888] text-xs mb-2">找到 {results.length} 筆結果</div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-[#aaa] text-xs">{s.id} · {s.industry}</div>
                        </div>
                        ...
                        <div className="w-8 flex justify-end">
                          <span className="w-8 h-8 rounded-full bg-[#ff9500] flex items-center justify-center">
